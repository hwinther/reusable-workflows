...
- `.github/workflows/` — reusable workflows (`on: workflow_call`) that compose the actions:
  ...
  - `resharper-cleanupcode.yml` — `workflow_dispatch`-style multi-formatter helper (filename is legacy from when it only handled ReSharper). Three modes, each a separate job gated by `inputs.mode`:

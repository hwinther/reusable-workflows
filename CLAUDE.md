...
- `.github/actions/` — composite actions (the building blocks):
  ...
  - `_push-image-with-signatures/` — **internal**. `docker tag` + `docker push` per tag, then `actions/attest` (SLSA build provenance), cosign keyless sign, and (when supply-chain attestation is on) cosign CycloneDX + vuln attestations. Used by `docker-container.yml`, `dotnet-container.yml`, and `image-push.yml`.
...
- `.github/workflows/` — reusable workflows (`on: workflow_call`) that compose the actions:
  ...
  - `format-and-create-pr.yml` — `workflow_dispatch`-style multi-formatter helper. Three modes, each a separate job gated by `inputs.mode`:
    - `cleanupcode` — runs `jb cleanupcode` against a .NET solution and opens a PR.
    - `ruff-format` — runs `ruff format <target_path>` against a Python tree (installs Ruff via `pip install ruff[==<ruff_version>]` after `actions/setup-python`) and opens a PR. Branch prefix `ruff/format-` so merged PRs are visually distinct from ReSharper ones.
    - `blame-ignore-revs` — formatter-agnostic. After a formatter PR has merged to the default branch, scans history for commits matching the caller-supplied `commit_subject` and appends their full SHAs to `.git-blame-ignore-revs`, opening a PR. Caller runs this once per formatter (with that formatter's `commit_subject`) — single-subject per run by design. Branch prefix is `jb/blame-ignore-revs-` (legacy — predates the ruff-format mode; kept to avoid churning consumer-side branch-protection patterns).
...
`stryker.yml`, `format-and-create-pr.yml` (when `mode: cleanupcode`), and `dependabot-update-dotnet-lockfiles.yml` all run `dotnet restore` against the consumer's solution and accept the same four NuGet inputs (`primary_nuget_source`, `extra_nuget_source*`) plus the optional `nuget_auth_token` secret. Each uses `_dotnet-add-nuget-source` as its "add extra source" step. `stryker.yml` additionally accepts `npm_registry_url` / `npm_scope` inputs and an `npm_auth_token` secret for its `node-stryker` job.

Caveat for `format-and-create-pr.yml`: the .NET/NuGet inputs (`solution_path`, `solution_name`, `dotnet_version`, `primary_nuget_source`, `extra_nuget_source*`, `nuget_auth_token`) and the `dotnet tool install JetBrains.ReSharper.GlobalTools` step apply only to `mode: cleanupcode`. The `mode: ruff-format` job consults a separate set of inputs (`python_version`, `ruff_version`, `target_path`) and installs Ruff via `pip` after `actions/setup-python` — no .NET toolchain is touched. The `mode: blame-ignore-revs` job is formatter-agnostic and only needs `commit_subject`. Still relevant when `mode: cleanupcode`: `dotnet tool install JetBrains.ReSharper.GlobalTools --global` resolves from nuget.org, so if `primary_nuget_source` redirects to a feed that doesn't upstream nuget.org the tool install breaks — point primary at a feed that proxies nuget.org or leave it empty.
